...
import com.longmai.dbsafe.engine.common.ClassHasher;
import com.longmai.dbsafe.engine.common.CustomHashedHashSet;
...
public class DbSafeOptionsRepository {

  private final Map<String, Object> map = new HashMap<String, Object>();

  private Set<DelayedOptionChange> delayedOptionChanges = new HashSet<DelayedOptionChange>();

  private List<DbSafeOptionChangedListener> listeners = new ArrayList<DbSafeOptionChangedListener>();
  ...
  <T> Object parse(Class<T> type, Object value) {
    if (type.isAssignableFrom(Boolean.class)) {
      return DbSafeUtil.isTrue(value.toString(), true);
...
//		}
      ...
      Object instance;
      try {
        instance = DbSafeUtil.forName(value.toString()).newInstance();
      } catch (Exception ex) {
  ...
  public <T> boolean setSet(Class<T> type, String key, String csv) {
    ...
    final List<String> collection = DbSafeUtil.parseCSVList(csv);
    ...
    if (collection.isEmpty()) {
		...
		if (type.equals(DbSafeFactory.class)) {
  ...
  void fireOptionChanged(final String key, final Object oldValue, final Object newValue) {
    if (initCompleted) {
      fireDelayedOptionChanges();
      for (DbSafeOptionChangedListener listener : listeners) {
  ...
  class DelayedOptionChange {
	...
	private DbSafeOptionsRepository getOuterType() {
		return DbSafeOptionsRepository.this;
	}
  }
}
