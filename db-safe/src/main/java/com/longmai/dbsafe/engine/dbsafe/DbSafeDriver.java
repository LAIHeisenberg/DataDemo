 ...
 */
public class DbSafeDriver implements Driver {
  private static Driver instance = new DbSafeDriver();
  private static JdbcEventListenerFactory jdbcEventListenerFactory;

  static {
    try {
      DriverManager.registerDriver(DbSafeDriver.instance);
  ...
  public Connection connect(String url, Properties properties) throws SQLException {
    ...
    DbSafeLogQuery.debug("this is " + this + " and passthru is " + passThru);

    final long start = System.nanoTime();

    if (DbSafeDriver.jdbcEventListenerFactory == null) {
      DbSafeDriver.jdbcEventListenerFactory = JdbcEventListenerFactoryLoader.load();
    }
    final Connection conn;
    final JdbcEventListener jdbcEventListener = DbSafeDriver.jdbcEventListenerFactory.createJdbcEventListener();
  ...
  protected Driver findPassthru(String url) throws SQLException {
    ...
    DbSafeModuleManager.getInstance();
  ...
  public static void setJdbcEventListenerFactory(JdbcEventListenerFactory jdbcEventListenerFactory) {
    DbSafeDriver.jdbcEventListenerFactory = jdbcEventListenerFactory;
  }
}
