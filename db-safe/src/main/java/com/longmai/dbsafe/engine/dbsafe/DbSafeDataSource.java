...
public class DbSafeDataSource implements DataSource, ConnectionPoolDataSource, XADataSource, Referenceable, Serializable {
   ...
   */
  public DbSafeDataSource(DataSource delegate) {
  ...
  protected synchronized void bindDataSource() throws SQLException {
    ...
    // Set any properties that the DbSafe.properties file contains
  ...
  private void setDataSourceProperties(Map<String, String> props) throws SQLException {
    ...
    // from the DbSafe.properties file contains any matching
    // name
    for (Method method : klass.getMethods()) {
      ...
      if (methodName.startsWith("set")) {
        ...
        // property in the list read in from DbSafe.properties.
          ...
          if (key.toLowerCase().equals(propertyName)) {
            try {
              ...
              if (paramType.isAssignableFrom(String.class)) {
                ...
                DbSafeLogQuery.debug("calling " + methodName + " on DataSource " + rdsName + " with " + value);
                ...
                DbSafeLogQuery.debug("method " + methodName + " on DataSource " + rdsName + " matches property "
                  + propertyName + " but expects unsupported type " + paramType.getName());
                matchedProps.put(key, value);
              }
            } catch (IllegalAccessException e) {
              throw new SQLException("DbSafe.properties file includes" + " datasource property " + key + " for datasource "
    ...
    // log properties defined in DbSafe.properties that were not found on the data source.
    for (String key : props.keySet()) {
      if (!matchedProps.containsKey(key)) {
        DbSafeLogQuery.debug("DbSafe.properties file includes" + " datasource property " + key + " for datasource " + rdsName
  ...
  public Reference getReference() throws NamingException {
    final Reference reference = new Reference(getClass().getName(), DbSafeDataSourceFactory.class.getName(), null);
  ...
  public PooledConnection getPooledConnection() throws SQLException {
    ...
    return new DbSafeXAConnection(castRealDS(ConnectionPoolDataSource.class).getPooledConnection(), this.jdbcEventListenerFactory);
  }
  ...
  public PooledConnection getPooledConnection(String user, String password) throws SQLException {
    ...
    return new DbSafeXAConnection(castRealDS(ConnectionPoolDataSource.class).getPooledConnection(user, password), this.jdbcEventListenerFactory);
  }
  ...
  public XAConnection getXAConnection() throws SQLException {
    ...
    return new DbSafeXAConnection(castRealDS(XADataSource.class).getXAConnection(), this.jdbcEventListenerFactory);
  }
  ...
  public XAConnection getXAConnection(String user, String password) throws SQLException {
    ...
    return new DbSafeXAConnection(castRealDS(XADataSource.class).getXAConnection(user, password), this.jdbcEventListenerFactory);
  }
